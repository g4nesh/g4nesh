<p>
  <img src="./assets/g4nesh-ascii.gif" alt="ascii plasma animation" width="100%">
</p>

hi! i'm ganesh and below you can find most of the projects i've worked on throughout the years (all the way back to 2021!)

for my full profile, visit [ganeshtalluri.com](https://ganeshtalluri.com/)

<!-- codex-token-counter:start -->
#### my codex usage across devices

<table width="100%">
  <thead>
    <tr>
      <th align="left" width="18%">stat</th>
      <th align="left" width="36%">value</th>
      <th align="right" width="46%">tokens over time</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td width="18%">lifetime tokens</td>
      <td width="36%">26,239,007,757 (26.24B)</td>
      <td rowspan="6" width="46%" valign="middle" align="right"><img src="./assets/codex-token-trend.svg" alt="Codex tokens over time" width="500"></td>
    </tr>
    <tr>
      <td>est. API cost</td>
      <td>$35,462.38</td>
    </tr>
    <tr>
      <td>current streak</td>
      <td>85 days</td>
    </tr>
    <tr>
      <td>daily history</td>
      <td>Sep 25, 2025 -&gt; Sep 24, 2026</td>
    </tr>
    <tr>
      <td>history tokens</td>
      <td>26,216,213,981</td>
    </tr>
    <tr>
      <td>updated</td>
      <td>Sep 24, 2026, 7:25 PM MST</td>
    </tr>
  </tbody>
</table>

<sub>OpenAI account lifetime counter; updates periodically when this Mac is available. Cost is an estimate using a fixed historical average, not a bill. The graph covers only returned daily history (26,216,213,981 tokens), which may differ from lifetime usage. <a href="./launchd/README.md#account-tokens-and-estimated-cost">Method and pricing</a>.</sub>
<!-- codex-token-counter:end -->
